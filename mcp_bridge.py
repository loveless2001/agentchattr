...
_MCP_INSTRUCTIONS = (
    ...
    "chat_read may return message objects with an attachments array. Treat attachments as part of the user's message, "
    "not optional metadata. Attachment objects may include markdown_text (inline extracted text), summary, file_path, "
    "original_file_path, and markdown_file_path. If a user asks whether you can see or read an uploaded file, inspect "
    "those fields and say clearly what you can access. For text-like uploads (txt/md/pdf/docx), prefer markdown_text "
    "first. For image uploads, you may only receive file metadata and local file paths, so do not pretend you already "
    "read image contents unless you actually inspect the file with your available tools.\n\n"
